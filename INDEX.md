...
- **2026-08-14** — [specs/2026-08-14-looptest-phase2-landscape-hori-anchor.md](specs/2026-08-14-looptest-phase2-landscape-hori-anchor.md) — Phase 2 landscape match to the hori anchor: records the landscape `LAYOUT` composition (RECORDED, loop r7), the separate-file decision, the anchor-cut assets (grinding wheel, cauldron, cleaned map), the init-view aspect bug fix, and the Opus-gated composition loop (r2 58 → r7 77; map 87 / bench 86, stopped at the 7-loop cap). `canon` (build spec, branch `sword-forge/dual-orientation-anchor-rework`).
