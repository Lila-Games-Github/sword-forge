...
- These specs describe the game **as implemented**, not aspirational design. As of 2026-08-12 the SSOT is a **two-build split** (see the `game-design.md` header): the ore→sword craft/movement loop (§2 steps 2–4, §3–§5) is canon per the canonical build `Swordforge_looptest_landscape.html` (the landscape loop-test; its portrait sibling `Swordforge_new_looptest.html` is retained but diverged, see `game-design.md` §9); economy/UI/onboarding (§6–§8) still reflect `index.html`. Forward-looking design lives in `../research/sword-forge-gdd.html` (v0.3), not here.
